...
function est_label=predictdtw(test_data,clusters,mel_vector_length,codebook)
est_label=zeros(size(test_data,1),1);
     for j=1:1:size(test_data,1)
         ...
         for z=1:clusters:size(codebook,1)
            ...
            temp1=(reshape(reshape(test_data(j,:)',1,[]),mel_vector_length,[]));
            dist=dtw(temp1,codebook(z:z+clusters-1,:)');
